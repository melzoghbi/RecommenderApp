...
public class RecommendationsApiWrapper
{
    ...
    public ModelInfo CreateModel(string modelName, string description = null)
    {
        ...
        modelName = modelName + "_" + DateTime.Now.ToString("yyyy-dd-M--HH-mm-ss");
    ...
    public CatalogImportStats UploadCatalog(string modelId, string catalogContentFile,string catalogDisplayName)
    {
        Console.WriteLine("Uploading catalog files ...");
        string uri = BaseUri + "/models/" + modelId + "/catalog?catalogDisplayName=" + catalogDisplayName;
       
        var response = _httpClient.PostAsync(uri, new StringContent(catalogContentFile)).Result;
    ...
    public UsageImportStats UploadUsage(string modelId, string usageContentFile, string usageDisplayName)
    {
        Console.WriteLine("Uploading " + usageDisplayName + " ...");

        string uri = BaseUri + "/models/" + modelId + "/usage?usageDisplayName=" + usageDisplayName;

        var response = _httpClient.PostAsync(uri, new StringContent(usageContentFile)).Result;
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception(
                String.Format("Error {0}: Failed to import usage data {1}, for model {2} \n reason {3}",
                    response.StatusCode, usageDisplayName, modelId, ExtractErrorInfo(response)));
        }
